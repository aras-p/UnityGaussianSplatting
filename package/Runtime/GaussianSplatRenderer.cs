...
namespace GaussianSplatting.Runtime
{
    class GaussianSplatRenderSystem
    {
        ...
        public void RenderPreparedSplats(CommandBuffer cmb, int eyeIndex)
        {
            ...
            foreach (var item in m_LastPreparedData.renderItems)
            {
                ...
                // Wait for compute shader to complete (fixes VR close-range ghosting)
    ...
    public class GaussianSplatRenderer : MonoBehaviour
    {
        ...
        // GPU fence to synchronize compute with render (fixes VR close-range ghosting)
        ...
        internal void CalcViewData(CommandBuffer cmb, Camera cam)
        {
            ...
            if (isStereo)
            {
                ...
                // Pass eye-specific view matrix (combined with object transform) for covariance calculation
                cmb.SetComputeMatrixParam(m_CSSplatUtilities, Props.ViewMatrixLeft, stereoViewLeft * matO2W);
                cmb.SetComputeMatrixParam(m_CSSplatUtilities, Props.ProjMatrixLeft, stereoProjLeft);
                ...
                // Pass eye-specific view matrix (combined with object transform) for covariance calculation
                cmb.SetComputeMatrixParam(m_CSSplatUtilities, Props.ViewMatrixRight, stereoViewRight * matO2W);
                cmb.SetComputeMatrixParam(m_CSSplatUtilities, Props.ProjMatrixRight, stereoProjRight);
            ...
            // Create fence to ensure compute completes before render (fixes VR close-range ghosting)
